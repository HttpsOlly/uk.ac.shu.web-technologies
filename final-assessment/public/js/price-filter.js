$(document).ready(function () {
    ...
    function filterEvents(events, minPrice, maxPrice, selectedGenre) {
        return events.filter(event => {
            ...
            const hasValidPrice = priceRange && priceRange.min !== undefined && priceRange.max !== undefined;
            const isInPriceRange = hasValidPrice ? priceRange.max >= minPrice && priceRange.min <= maxPrice : minPrice === 0 && maxPrice === 0;
            const isAGenreMatch = selectedGenre ? genre === selectedGenre : true;

            return isInPriceRange && isAGenreMatch;
        });
    }
    ...
    function displayEvents(events) {
        ...
        if (events.length === 0) {
            eventsContainer.append("<p>No events found for these parameters!</p>");
        } else {
